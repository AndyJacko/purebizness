...
if ($_GET["o"] == "flagged") {
	...
	$heading = "Flagged";
} elseif ($_GET["o"] == "failed"){
	$query_RSDirectory = "SELECT * FROM tbl_purebiz_listings WHERE list_override=0 AND list_flagged=0 ORDER BY list_active,list_name ASC";
	$heading = "Failed (NON-FLAGGED, NON-OVERRIDDEN)";
} elseif ($_GET["o"] == "override"){
	$query_RSDirectory = "SELECT * FROM tbl_purebiz_listings WHERE list_override=1 ORDER BY list_active,list_name ASC";
	$heading = "Overridden";
} else {
  ...
  <section id="pagebody">
    <div id="content" class="column">
      ...
      <!-- InstanceBeginEditable name="content" -->
        <div id="content">
          <h1><?php echo $heading; ?> Reciprocal Links</h1>
          <br><br>
          ...
          <?php if ($row_RSDirectory["list_override"] != 1) {?>
					<?php if ($_GET["o"] == "failed") {?>
          <script language="javascript" type="text/javascript"> checkrecip2('<?php echo $theRecip; ?>',<?php echo $row_RSDirectory["tbl_id"]; ?>) </script> 
          <?php } else { ?>
