...
if ($_GET["s"] == 0) {
  $updateSQL = "UPDATE tbl_purebiz_listings SET list_active=1 WHERE tbl_id=".$_GET["tbl_id"];
  mysql_select_db($database_Wisdom_Mcr, $Wisdom_Mcr);
  $Result1 = mysql_query($updateSQL, $Wisdom_Mcr) or die(mysql_error());
	
	$d8 = gmdate("d/m/Y");
  $updateSQL = "UPDATE tbl_purebiz_dir_scats SET sub_updated='$d8' WHERE tbl_id=".$row_RSListing["list_sub"];
  mysql_select_db($database_Wisdom_Mcr, $Wisdom_Mcr);
  $Result1 = mysql_query($updateSQL, $Wisdom_Mcr) or die(mysql_error());
	...
	<table width='550' border='0' cellspacing='0' cellpadding='0'>
		<tr>
			<td align='justify' style='font-family: Arial; font-size: 12px;'>
				...
				<p>We are pleased to inform you that your Purebizness listing has been activated and is now viewable on our website.</p>
  ...
  $updateSQL = "UPDATE tbl_purebiz_listings SET list_active=0 WHERE tbl_id=".$_GET["tbl_id"];
  mysql_select_db($database_Wisdom_Mcr, $Wisdom_Mcr);
  $Result1 = mysql_query($updateSQL, $Wisdom_Mcr) or die(mysql_error());
