...
if(strlen($large_photo_exists)>0 && strlen($thumb_photo_exists)>0){
	clearstatcache();
	if(!is_dir($_SESSION["dir"]."/".$_SESSION["sub"]."/".$_SESSION["pname"]."/")){
		mkdir($_SESSION["dir"]."/".$_SESSION["sub"]."/".$_SESSION["pname"]."/");
		mkdir($_SESSION["dir"]."/".$_SESSION["sub"]."/".$_SESSION["pname"]."/m/");
		mkdir($_SESSION["dir"]."/".$_SESSION["sub"]."/".$_SESSION["pname"]."/t/");
	}
  clearstatcache();
  ...
  $updateSQL = "UPDATE tbl_purebiz_listings SET list_ppic='".$usrCode.".jpg"."' WHERE tbl_id=".$_SESSION["id"];
  ...
  unset($_SESSION['popo']);
	...
	if ($_SESSION["ptype"] == 3 || $_SESSION["ptype"] == 4) {
		$loc = "/adultdirectories/addmorepix.php";
	} else {
		unset($_SESSION['dir']);
		unset($_SESSION['sub']);
		unset($_SESSION['pname']);
		$loc = "/adultdirectories/profileadded.php";
	}
...
<body>
  ...
  <section id="pagebody">
    <div id="content" class="column">
      ...
      <br><br>
      <p><em>Note: Max image size is 1000px width and will be resized automatically to 200px x 268px - Max image filesize is 1mb</em>.</p>
      <p>To get help creating the correct size image, go <a href="http://www.picresize.com/" target="_blank">here</a>.</p>
      <br><br>
      <strong>IMPORTANT! Click and drag on your uploaded image to create your thumbnail. Preview of thumbnail will be shown, when happy click "SAVE THUMBNAIL"</strong>
      <br><br>
      <?php
      ...
      <?php if (!isset($_SESSION["popo"])) { ?>
        <div id="addpic" class="addpicform">
          <form name="photo" enctype="multipart/form-data" action="<?php echo $_SERVER["../PHP_SELF"];?>" method="post">
            <label>Main Picture<span class="small">Main profile picture & thumbnail</span></label>
