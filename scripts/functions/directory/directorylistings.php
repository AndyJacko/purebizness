...
switch ($_SESSION["country"]) {
  case "all":	
    $query_RSListings = "SELECT * FROM tbl_purebiz_listings WHERE list_active=1 AND list_sub=".$row_RSSubName["tbl_id"]." ORDER BY ".$_SESSION["order"]." ".$_SESSION["direction"];
	break;
  default:	
    $query_RSListings = "SELECT * FROM tbl_purebiz_listings WHERE list_active=1 AND list_sub=".$row_RSSubName["tbl_id"]." AND list_country='".$_SESSION["country"]."' ORDER BY ".$_SESSION["order"]." ".$_SESSION["direction"];
}
$query_limit_RSListings = sprintf("%s LIMIT %d, %d", $query_RSListings, $startRow_RSListings, $_SESSION["maxrows"]);
$RSListings = mysql_query($query_limit_RSListings, $con) or die(mysql_error());
  ...
  $listings .= "</div>";
	if ($totalRows_RSSponsored > 0) {
		...
		if ($totalRows_RSSponsored > 1) {
			for ($i=0;$i < 2; $i++) {
				$listings .= "<div class='dirlisting sponbg'><div class='dirlistpic'><a href='/adultdirectories/viewdetails.php?p=".$row_RSSponsored["tbl_id"]."&name=".strtolower($row_RSSponsored["list_name"])."'><img src='/adultdirectories/$dir/$sub/".$row_RSSponsored["list_title"]."/t/".$row_RSSponsored["list_ppic"]."'></a></div>";	
				$listings .= "<div class='dirinfo sponbg'>".getflag(strtoupper($row_RSSponsored["list_country"]))."&nbsp;&nbsp;<a href='/adultdirectories/viewdetails.php?p=".$row_RSSponsored["tbl_id"]."&name=".$row_RSSponsored["list_name"]."'>".strtoupper($row_RSSponsored["list_name"])."</a>&nbsp;&nbsp;<strong>COUNTRY:</strong> ".strtoupper($row_RSSponsored["list_country"])."&nbsp;&nbsp;<strong>AREA:</strong> ".strtoupper($row_RSSponsored["list_area"]). "<br>";
	...
	do {	
    $listings .= "<div class='dirlisting'><div class='dirlistpic'><a href='/adultdirectories/viewdetails.php?p=".$row_RSListings["tbl_id"]."&name=".strtolower($row_RSListings["list_name"])."'><img src='/adultdirectories/$dir/$sub/".$row_RSListings["list_title"]."/t/".$row_RSListings["list_ppic"]."'></a></div>";	
	  $listings .= "<div class='dirinfo'>".getflag(strtoupper($row_RSListings["list_country"]))."&nbsp;&nbsp;<a href='/adultdirectories/viewdetails.php?p=".$row_RSListings["tbl_id"]."&name=".$row_RSListings["list_name"]."'>".strtoupper($row_RSListings["list_name"])."</a>&nbsp;&nbsp;<strong>COUNTRY:</strong> ".strtoupper($row_RSListings["list_country"])."&nbsp;&nbsp;<strong>AREA:</strong> ".strtoupper($row_RSListings["list_area"]). "<br>";
